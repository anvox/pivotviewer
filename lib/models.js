...
KPivotViewer.Models.filter_by_facet = function(facet_index, data) {
  ...
  if(KPivotViewer.Models.facets[facet_index].type == 'tag'){
    return KPivotViewer.Models.filter_by_tag_facet(facet_index,data);
  }
  return false;
}
...
KPivotViewer.Models.filter_by_tag_facet = function(facet_index, data) {
  var result = [];
  for (var i = KPivotViewer.Models.source.length - 1; i >= 0; i--) {
    if( KPivotViewer.Models.is_array_intersect(KPivotViewer.Models.source[i].facets[facet_index],data) ){
      result.push(KPivotViewer.Models.source[i].id);
    }
  }
